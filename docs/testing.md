...
# Expenses import
python3 _dev/export-timesheet.py \
  --output /tmp/expenses.xlsx \
  --expenses Raw/2026/05-20-2026_expenses.csv \
  --rates timesheet-rates.csv
```
...
- **Expenses:** rows from the expense CSV appear under **Expenses** on the matching worker sheet; `Reimbursable = No` rows do not add to payroll.
- **Total $:** equals `Subtotal − Withheld + Expense reimbursements` when expenses are present; otherwise `Subtotal − Withheld`.
...
| Parser logic (Notion/Turno/Timeclock/Expenses) | Run the matching smoke scenarios above and open each workbook. |
| Workbook structure / section order / formulas | Run the smoke scenarios + diff a previous workbook to confirm only intended cells changed. |
| Rate / withholding / allowance rules | Run the smoke scenarios + add a hand-computed expected total for at least one worker and compare. |
